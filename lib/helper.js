import api from '../src/api/poselink';
import serviceMap from './services';

let services = [];

export const loadServices = async () => {
  loadedServices = await api.getServices();
  loadedServices.forEach(service => {
    let Service = serviceMap[service.type][service.name];

    services.push(new Service({
      classification: service.classification,
      id: service.id,
      type: service.type,
      name: service.name,
      icon: service.icon,
      events: service.events
    }));
  });
};

export const reloadServices = async () => {
  services.forEach(service => {
    if(service.type == 'trigger') {
      service.destroy();
    }
  });
  services.length = 0;
  await loadServices();
};

export const reloadService = async(id) => {
  services = services.filter(service => {
    if(service.type == 'trigger' && service.id == id) {
      service.destroy();
    }
    return service.id != id
  });

  await loadService(id);
}

const loadService = async(id) => {
  loadedServices = await api.getServices();
  loadedServices.forEach(service => {
    if (service.id == id) {
      let Service = serviceMap[service.type][service.name];
      services.push(new Service({
        classification: service.classification,
        id: service.id,
        type: service.type,
        name: service.name,
        icon: service.icon,
        events: service.events
      }));
    }
  });

  await loadServiceConfigs();
};

export const getServices = () => {
  let newServices= services.map(service=>{
    return {
      id: service.id,
      name: service.name,
      icon: service.icon,
      classification: service.classification,
      isConnected: service.isConnected()
    }
  });

  return newServices;
};

export const disconnectAllService = async () => {
  services.forEach(service => {
    service.disconnect();
  });
};

export const loadServiceConfigs = async () => {
  let configs = await api.getUserServiceConfigs();

  configs.forEach(config => {
    if (config.status == 'connected') {
      let service = getServiceById(config.serviceId);

      service.connect();
      service.start();
    }
  });
};

export const getServiceById = (id) => (
  services.reduce((acc, service) => {
    if (service.id == id) {
      return service;
    }
    return acc;
  }, null)
);

export const getServiceByTypeName = (type, name) => {
  let filterServices = services.filter(service => (
    (service.type == type) && (service.name == name)
  ));
  return filterServices[0] || null;
};

export const getServiceByEventId = (id) => {
  let filterServices = services.filter(service => (
    service.events.filter(event => event.id == id)
  ));
  return filterServices[0] || null;
};

export const getEventById = (id) => {
  let filterEvent = [];
  let filterServices = services.filter(service => (
    service.events.map(event => {
      if(event.id == id) {
        filterEvent = event;
      }
       return event;
    })
  ));
  return filterEvent || null;
}

export const getEventsByServiceId = (id) => {
  let filterServices = services.filter(service => (
    service.id == id
  ));

  return filterServices[0].events || null;
}

export const getTriggerService = () => {
  let filterServices = services.filter(service => (
    service.type == 'trigger'
  ));
  return filterServices || null;
};

export const getActionService = () => {
  let filterServices = services.filter(service => (
    service.type == 'action'
  ));
  return filterServices || null;
};
